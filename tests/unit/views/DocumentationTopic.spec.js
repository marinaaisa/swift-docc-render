...
describe('DocumentationTopic', () => {
  ...
  it('sets enableNavigator to true if schemaVersion is compatible', async () => {
    ...
    expect(wrapper.find(NavigatorSidebar).props('enableNavigator')).toBe(false);
    ...
    expect(wrapper.find(NavigatorSidebar).props('enableNavigator')).toBe(true);
  });

  it('passes a technology to the NavigatorSidebar when no reference is found for a top-level collection', () => {
    const technologies = {
    ...
    const navigatorSidebarComponent = wrapper.find(NavigatorSidebar);
    expect(navigatorSidebarComponent.exists()).toBe(true);
    // assert the technology is the last fallback
    expect(navigatorSidebarComponent.props('technology')).toEqual({
  ...
  it('finds the parentTopicIdentifiers, that have the closest url structure to the current page', () => {
    ...
    expect(wrapper.find(NavigatorSidebar).props('parentTopicIdentifiers'))
